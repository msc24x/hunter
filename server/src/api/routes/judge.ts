...
import { HunterExecutable, resCode } from "../../environments/environment";
        ...
        (questions)=>{
          ...
          models.competitions.findAll(
            ...
            async competitions=>{
              ...
              if(!models.competitions.isLiveNow(hunterExecutable.for.competition_id) || !models.competitions.hasNotEnded(competitions[0].start_schedule, competitions[0].duration)){
              ...
              try{
                let stdout = await judgeService.execute(hunterExecutable,samples,questions[0],user)

                if(!samples)
                  scoreboardService.updateResult(user, hunterExecutable, stdout[0] == '1', questions[0].points)
